...
$query = $mysqli->query("SELECT * FROM tb_absenpin WHERE id_pegawai='$_SESSION[id_pegawai]' AND tanggal='$tanggal'");
$query2 = $mysqli->query("SELECT * FROM tb_absenpin WHERE tanggal LIKE '%" . date('m-Y') . "'");
$data = $query->fetch_assoc();

if (isset($_POST['export'])) {
    $queryexport = "SELECT * FROM tb_absenpin WHERE tanggal LIKE '%" . $_POST['export_tahun'] . "'";
    if ($_POST['export_bulan'] > 0) {
        $queryexport = "SELECT * FROM tb_absenpin WHERE tanggal LIKE '%" . $_POST['export_bulan'] . "-" . $_POST['export_tahun'] . "'";
...
if (isset($_POST['masuk'])) {
    if ($data) {
        ...
        $res =  mysqli_query($konek, "INSERT INTO tb_absenpin SET id_pegawai='$id', tanggal='$tanggal', jam_masuk= '$jamSekarang'");
...
if (isset($_POST['pulang'])) {
    if ($data['jam_pulang']) {
        ...
        $res =  mysqli_query($konek, "UPDATE tb_absenpin SET jam_pulang='$jamSekarang' WHERE id_absenpin ='" . $data['id_absenpin'] . "' ");
        echo '<script>alert("terima kasih");history.go(-1);</script>';
    }
}
        ...
        <div class="row mb2">
            ...
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <li class="breadcrumb-item"><a href="?page=jabatanread">Absen</a></li>
                </ol>
            </div>
        </div>
    </div>
