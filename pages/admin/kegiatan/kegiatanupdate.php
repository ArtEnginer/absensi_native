...
if (isset($_GET['id_kegiatan'])) {
        ...
        $id = $_GET['id_kegiatan'];
        $findSql = "SELECT * FROM kegiatan WHERE id_kegiatan = ?";
        $stmt = $db->prepare($findSql);
        $stmt->bindParam(1, $_GET['id_kegiatan']);
        $stmt->execute();
        $row = $stmt->fetch();
        if (isset($row['id_kegiatan'])) {
                if (isset($_POST['button_update'])) {
                        ...
                        $id = $_GET['id_kegiatan'];
                        $ValidateSql = "SELECT * FROM kegiatan WHERE judul = ? AND id_kegiatan != ?";
                        $stmt = $db->prepare($ValidateSql);
                        $stmt->bindParam(1, $_GET['judul']);
                        $stmt->bindParam(2, $_GET['id_kegiatan']);
                        $stmt->execute();
                        if ($stmt->rowCount() > 0) {
?>
                                <div class="alert alert-danger alert-dismissible">
                                        ...
                                        Judul Kegiatan Sudah Ada
                                </div>
        <?php
                        } else {
                                $updateSql = "UPDATE kegiatan SET nomor = ? , judul = ?, id_pegawai = ?, lokasi = ?, waktu = ?, dana = ? WHERE id_kegiatan = $id";
                ...
                <div class="card">
                        ...
                        <div class="card-body">
                                ...
                                <div class="form-group">
                                                ...
                                                <input type="text" value="<?php echo $row['nomor'] ?>" class="form-control" name="nomor" readonly>
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                                <label for="id">Penanggung Jawab</label>
                                        ...
                                        <div class="form-group">
                                                ...
                                                <input type="text" value="<?php echo $row['lokasi'] ?>" class="form-control" name="lokasi" required>
                                        </div>
                                        <div class="form-group">
                                                <label for="waktu">Waktu Pelaksanaan</label>
                                                <input type="date" value="<?php echo $row['waktu'] ?>" class="form-control" name="waktu" required>
                                        </div>
                                        <div class="form-group">
